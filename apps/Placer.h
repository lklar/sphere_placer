...
public:
	/*SpherePlacer(std::vector<Sphere>& inputSpheres, std::function<double(const Sphere&)> sphereToFillingFactor) :
		inputSpheres(&inputSpheres),
		dfi(new dynamicFfInterface(sphereToFillingFactor))
	{};

	SpherePlacer(std::vector<Sphere>& inputSpheres, double extraTheta, int N_phi) :
		extraTheta(extraTheta),
		N_phi(N_phi),
		inputSpheres(&inputSpheres) {};*/

	SpherePlacer(PlacerConfig& placerConfig);

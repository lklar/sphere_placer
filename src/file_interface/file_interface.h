...
public:
	...
	int load(std::filesystem::path fPath);

	int load_from_drive(std::filesystem::path fPath);

	int save(std::filesystem::path fPath);
	...
	long long get_row_count(std::string tableName);

	std::vector<double3> read_positions(std::string tableName, std::string conditions = "");

	int read_doubles(std::string query, std::vector<std::vector<double>>& data);

	std::map<std::string, double> read_parameters();

	void write_doubles(std::string insertQuery, std::vector<std::vector<double>>& data);

	void write_row(std::string tableName, std::vector<std::pair<std::string, double>> data);

	void write_positions(std::string tableName, std::vector<double3>& positions);

	void write_pos_vel(std::string tableName, std::vector<double3>& positions, double3 velocity);

	void write_parameters(std::map<std::string, double>& params);
